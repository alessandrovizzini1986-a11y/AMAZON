...
import { FINE_TONE } from "./constants";
...
export default async function FinesPage({
  ...
  searchParams: Promise<{ stato?: string; assegnazione?: string; station?: string }>;
}) {
  ...
  const [fines, stations] = await Promise.all([
    db.fine.findMany({
      where,
      ...
      {fines.length === 0 ? (
        ...
        <div className="card overflow-x-auto">
          <table className="table-base">
            <thead>
              <tr>
                ...
                <th>Importo</th><th>Punti</th><th>Conducente</th><th>Stato</th><th>Ricorso entro</th>
              </tr>
            </thead>
            <tbody>
              {fines.map((f) => (
          ...
          <div className="px-3 pb-3">
            <SourceNote>tabella Fine ({scopeLabel}), ultime 200 righe per data infrazione decrescente</SourceNote>
          </div>
        </div>
      )}
    </div>
