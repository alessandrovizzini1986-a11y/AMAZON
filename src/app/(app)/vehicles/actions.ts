...
const vehicleSchema = z.object({
  ...
  alimentazione: z.enum(["DIESEL", "DIESEL_HVO", "BENZINA", "ELETTRICO", "METANO", "GPL", "IBRIDO"]),
  hvoCompatibile: z.coerce.boolean().optional(),
  immatricolazione: z.coerce.date(),
  stationId: z.string().min(1),
  stato: z.enum(["ATTIVO", "IN_OFFICINA", "SOSTITUTIVO", "DISMESSO"]).default("ATTIVO"),
  kmAttuali: z.coerce.number().int().min(0).default(0),
  canoneGiorno: z.coerce.number().min(0),
...
export async function updateVehicleAction(vehicleId: string, formData: FormData) {
  ...
  await audit({
    ...
    entityId: vehicleId,
    meta: {
      before: { stato: before.stato, kmAttuali: before.kmAttuali, canoneGiorno: String(before.canoneGiorno) },
      after: { stato: data.stato, kmAttuali: data.kmAttuali, canoneGiorno: String(data.canoneGiorno) },
    },
  });
